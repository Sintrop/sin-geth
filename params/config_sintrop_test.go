...
// TestGenesisHashSintrop tests that SintropGenesisHash is the correct value for the genesis configuration.
func TestGenesisHashSintrop(t *testing.T) {
	genesis := DefaultSintropGenesisBlock()
	block := genesisToBlock(genesis, nil)
	if block.Hash() != SintropGenesisHash {
		t.Errorf("want: %s, got: %s", SintropGenesisHash.Hex(), block.Hash().Hex())
	}
}
